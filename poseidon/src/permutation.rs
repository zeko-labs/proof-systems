...
mod sp1 {
    ...
    fn full_round_sp1(
        ...
        for row in 0..3 {
            ...
            for col in 0..3 {
                acc = add(acc, mul(mds[row][col], tmp[col], modulus), modulus);
            }
            state[row] = acc;
        }
        ...
        // Add round constants.
        for i in 0..3 {
            state[i] = add(state[i], rc[i], modulus);
        }
    }
    ...
    >(
        ...
        // Convert the output state back once.
        for i in 0..3 {
            state[i] = to_ark(s[i]);
        }
    }
